...
import { Loader2, Plus, X, QrCode, Instagram, Linkedin, MessageCircle, Heart, User } from 'lucide-react';
...
export default function MePage() {
  ...
  const [profile, setProfile] = useState<Yesenter | null>(null);
  ...
  const onSubmit = async (data: ProfileForm) => {
    ...
    try {
      ...
      setProfile({ ...profileData, id: user.uid } as Yesenter);
      setSuccess(true);
        ...
        <Card>
          ...
          <CardContent className="text-center space-y-4">
            <div className="bg-white p-4 rounded-lg inline-block">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`${window.location.origin}/unlock/${user?.uid}`)}`}
                alt="Profile QR Code"
                className="w-48 h-48"
              />
            </div>
